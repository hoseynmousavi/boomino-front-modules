...
function OnBoardingBtn({slideIndex, setSlide})
{
    ...
    return (
        <Material className={`on-boarding-next-cont ${slideIndex === 1 ? "done" : ""}`} backgroundColor={createMaterialColor({variable: slideIndex === 1 ? "--solid-light" : "--first-color"})} onClick={goNext}>
            <KeyboardArrowSvg className={`on-boarding-next ${slideIndex === 1 ? "hide" : ""}`}/>
            <div className={`on-boarding-next-text ${slideIndex === 1 ? "hide" : ""}`}>بعدی</div>
            <div className={`on-boarding-done ${slideIndex === 1 ? "show" : ""}`}>برو بریم</div>
        </Material>
    )
}
