...
function AlertModalButtons({isLoading, cancelBtn, onSubmit, submitBtn, isOk = true, submitDisable, onDisableClick})
{
    return (
        <div className="alert-buttons">
            ...
            <Material className={`alert-button ${submitDisable ? "disable" : ""} ${isOk ? "ok" : "yes"}`} disable={submitDisable || isLoading} backgroundColor={createMaterialColor({variable: isOk ? "--toast-fail-text" : "--first-color"})} onDisableClick={onDisableClick} onClick={onSubmit}>
                {
                    isLoading ?
                        <MyLoader width={20}/>
                        :
